...
export default function ConversationView({ hideHeader = false }: Props) {
  ...
  return (
    <div className={`flex flex-col h-full ${hideHeader ? "" : "max-h-[calc(100vh-100px)]"}`}>
      {/* 顶栏：panel 模式下隐藏 */}
      {!hideHeader && (
      ...
      <div
        ref={scrollRef}
        className={`flex-1 overflow-y-auto py-5 space-y-4 pr-1 ${hideHeader ? "px-4" : ""}`}
      >
        {messages.map((m, i) =>
          m.role === "user" ? (
      ...
      {/* 输入框 */}
      <div className={`pt-3 border-t border-[#e5e7eb] ${hideHeader ? "px-4 pb-4" : ""}`}>
        <AgentInputBox
          size="md"
          placeholder="继续提问..."
